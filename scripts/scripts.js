...
if (!CSS.supports('animation-timeline: view()') && window.matchMedia('(prefers-reduced-motion: no-preference)').matches) {
  ...
  LINES.forEach((LINE, index) => {
    gsap.from(LINE, { 
      ...
      opacity: 0,
      scrollTrigger: {
        trigger: 'section:nth-of-type(2) article:nth-of-type(3) section:nth-of-type(4)',
        scrub: 0.5,
...
// Add this code to your scripts.js file or wherever you include your GSAP script

// Mobile-specific GSAP adjustments
function initMobileGSAP() {
  // Only run on mobile devices
  if (window.innerWidth <= 768) {
    // Check if GSAP and ScrollTrigger are loaded
    if (typeof gsap !== 'undefined' && typeof ScrollTrigger !== 'undefined') {
      // Adjust timeline animations for mobile devices
      
      // Fix content height for mobile
      gsap.set('section:nth-of-type(2) article:nth-of-type(3)', {
        height: '300vh' // Shorter on mobile
      });
      
      // Move heading position up for better visibility
      gsap.set('section:nth-of-type(2) article:nth-of-type(3) h2', {
        marginTop: '40vh' // Adjusted for mobile
      });
      
      // Adjust the text blocks animation timing
      const LINES = document.querySelectorAll('.text-blocks p');
      LINES.forEach((LINE, index) => {
        // Clear any existing animations
        ScrollTrigger.getAll().forEach(st => {
          if (st.vars.trigger === 'section:nth-of-type(2) article:nth-of-type(3) section:nth-of-type(4)') {
            st.kill();
          }
        });
        
        // Create new animations with mobile-friendly timings
        gsap.from(LINE, { 
          yPercent: 100,
          opacity: 0,
          scrollTrigger: {
      ...
      // Fix image positioning issues on mobile
      gsap.set('.fixed img', {
        left: '0',
        translate: '0 0',
        transform: 'none'
      ...
      // Adjust final section clip path for better mobile display
      gsap.set('section:nth-of-type(2) article:last-of-type .fixed', {
        clipPath: 'ellipse(220% 200% at 50% 260%)',
      });
    }
  }
}
...
window.addEventListener('load', initMobileGSAP);
window.addEventListener('resize', initMobileGSAP);